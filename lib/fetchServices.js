...
export async function fetchForecast(searchCity) {
    try {
        ...
        return {
            ...
            city,
            conditions,
            snowTotal: Number(detailedStats.snowSum),
            snowDays: Number(detailedStats.daysOfSnow),
            rainTotal: Number(detailedStats.rainSum),
            rainDays: Number(detailedStats.daysOfRain),
            tempAvg: Number(detailedStats.tempAvg),
            yearlyMinTemp: Number(detailedStats.yearlyMinTemp),
            yearlyMaxTemp: Number(detailedStats.yearlyMaxTemp),
        };
    } catch (error) {
        console.log(error);
    }
...
async function fetchRainSnowAvg(searchCity) {
    try {
        let cityParams = await axios.get(
            `https://api.openweathermap.org/geo/1.0/direct?q=${searchCity}&limit=5&appid=e63a54f308d51d63317b621826a180fe`
        );
